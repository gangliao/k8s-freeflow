...
ShmPiece *FreeFlowRouter::initCtrlShm(const char *tag)
{
    ...
    if (!sp) LOG_ERROR("Failed to create control shm for tag  " << tag);
...
void FreeFlowRouter::start()
{
    ...
    pthread_t hosts_th;
    pthread_create(&hosts_th, NULL,
                   [](void *ptr) {
                       static_cast<FreeFlowRouter *>(ptr)->updateHostList();
                       return (void *)nullptr;
                   },
                   this);
    ...
    pthread_t ipmap_th;
    pthread_create(&ipmap_th, NULL,
                   [](void *ptr) {
                       static_cast<FreeFlowRouter *>(ptr)->updateVipMap();
                       return (void *)nullptr;
                   },
                   this);
                ...
                if (qp_csp->state == REQ_DONE)
                {
                    ...
                    switch (req_header->func)
                    {
                        case IBV_POST_SEND:
                        {
                            ...
                            for (int i = 0; i < post_send->wr_count; i++)
                            {
                                ...
                                if (wr[i].num_sge > 0)
                                {
                                    ...
                                    for (int j = 0; j < wr[i].num_sge; j++)
                                    {
                                        ...
                                        { uint32_t stime = sge[j].length *
                                        1000000 / MAX_QP_RATE_LIMIT; if (stime)
                                        { usleep(stime);
                                            }
                                            else {
                                                usleep(1);
                            ...
                            if (bad_wr == NULL)
                            {
                                // this IF is not needed right now, but left
                                // here for future use
    ...
    while (1)
    {
        ...
        switch (header.func)
        {
            ...
            case IBV_ALLOC_PD:
            {
                ...
                {
                    LOG_INFO(
                        "PD handle is no less than MAX_QUEUE_MAP_SIZE. "
                        "pd_handle="
                        << pd->handle);
                }
                else
            ...
            case IBV_REG_MR:
            {
                ...
                {
                    LOG_ERROR(
                        "Failed to regiester the MR. Current shared memory "
                        "size: "
                        << sp->size);
                    goto end;
                }
            ...
            case IBV_POST_SEND:
            {
                ...
                if (bad_wr == NULL)
                {
                    // this IF is not needed right now, but left here for future
                    // use
                    if (post_send->wr_count == wr_success)
                    {
                        ((struct IBV_POST_SEND_RSP *)rsp)->bad_wr = 0;
            ...
            case IBV_CREATE_AH:
            {
                ...
                {
                    LOG_INFO(
                        "AH handle is no less than MAX_QUEUE_MAP_SIZE. "
                        "ah_handle="
                        << ah->handle);
                }
                else
            ...
            case IBV_CREATE_SRQ:
            {
                ...
                {
                    LOG_INFO(
                        "SRQ handle is no less than MAX_QUEUE_MAP_SIZE. "
                        "srq_handle="
                        << srq->handle);
                }
                else
            ...
            case CM_CREATE_EVENT_CHANNEL:
            {
                ...
                {
                    LOG_INFO(
                        "channel id is no less than MAX_QUEUE_MAP_SIZE. "
                        "channel_id="
                        << event_channel->fd);
                }
                else
