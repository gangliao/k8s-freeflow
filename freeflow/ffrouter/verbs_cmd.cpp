...
int ibv_exp_cmd_query_device_resp(int cmd_fd, void *cmd_in, void *resp_out)
{
    ...
    IBV_INIT_CMD_RESP_EXP(QUERY_DEVICE, &cmd, sizeof(cmd), 0, &resp,
                          sizeof(resp), 0);
...
int ibv_cmd_query_qp_resp(int cmd_fd, void *cmd_in, int cmd_size,
                          void *resp_out)
{
    char cmd_inp[100];
...
int ibv_cmd_create_flow_resp(int cmd_fd, void *cmd_in, int written_size,
                             int exp_flow, void *resp_out)
{
    char cmd_inp[1024];
    ...
    printf("exp_flow=%d, written_size=%d, num_of_specs: %d\n", exp_flow,
           written_size, cmd->flow_attr.num_of_specs);

    int i = 0;
    for (i = 0; i < cmd->flow_attr.num_of_specs; i++)
    {
        printf("spec.size=%d, spec.type=%x\n",
               ((struct ibv_kern_spec *)ib_spec_ptr)->hdr.size,
               ((struct ibv_kern_spec *)ib_spec_ptr)->hdr.type);
        ib_spec_ptr =
            ib_spec_ptr + ((struct ibv_kern_spec *)ib_spec_ptr)->hdr.size;
    }
